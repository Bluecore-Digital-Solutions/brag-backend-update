...
namespace Brag.SharedServices.Helpers
{
    ...
    {//MD5
        ...
        public CryptionManagerMD5(IOptions<AppSettings> settings)
        {
            ...
            EncryptionKey = appSettings.EfcKey;
        }
        public static string Encrypt(string unencryptedText)
        {
            if (string.IsNullOrEmpty(unencryptedText))
                throw new ApiGenericException("You cannot encrypt an empty text", "XRPT001");
        ...
        public static string Decrypt(string encryptedText)
        {
            if (string.IsNullOrEmpty(encryptedText))
                throw new ApiGenericException("You cannot decrypt an empty text", "XRPT002");
        ...
        public static string Encrypt(string unencryptedText, string Ekey)
        {
            if (string.IsNullOrEmpty(unencryptedText))
                throw new ApiGenericException("You cannot encrypt an empty text", "XRPT001");

            var hashmd5 = new MD5CryptoServiceProvider();
            var hashedKeyBytes = hashmd5.ComputeHash(Encoding.UTF8.GetBytes(Ekey));
        ...
        public static string Decrypt(string encryptedText, string Ekey)
        {
            if (string.IsNullOrEmpty(encryptedText))
                throw new ApiGenericException("You cannot decrypt an empty text", "XRPT002");
